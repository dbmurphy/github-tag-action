...
function getClosedPRCommits() {
    ...
    if (!('pull_request' in github_2.context.payload)) {
        ...
        for (let i = 0; i++; i <= pr_commit_count) {
            core.info(querystring_1.stringify({
                message: JSON.parse(github_2.context.payload.commits)[i].message,
                sha: JSON.parse(github_2.context.payload.commits)[i].sha
            }));
        }
        if (pr_commit_count) {
            commits = github_2.context.payload.commits
