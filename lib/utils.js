...
function getClosedPRCommits() {
    ...
    if ('pull_request' in github_2.context.payload) {
        core.info("Foobar:::" + JSON.stringify(github_2.context.payload.commits));
        let pr_commit_count = JSON.parse(github_2.context.payload.commits).length;
        core.info("We found " + pr_commit_count + " commits from the PR.");
        if (pr_commit_count) {
            commits = JSON.parse(github_2.context.payload.commits)
